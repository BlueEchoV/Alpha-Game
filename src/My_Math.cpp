...
// North is zero degrees and rotates clockwise.
float calculate_facing_direction_north_clockwise(V2 vec) {
	...
	// Shift to 0° north, counterclockwise
    angle = fmod(90.0f - angle + 360.0f, 360.0f);

    return angle;
}

// East is zero degrees and rotates counterclockwise.
float calculate_facing_direction_east_counterclockwise(V2 vec) {
	...
	// atan2(y, x) gives angle from east, CCW (-180° to 180°)
    float angle_rad = atan2(vec.y, vec.x);
	// Convert to degrees
    float angle_deg = angle_rad * (float)(180.0 / M_PI);  

    // Normalize to 0°–360° (positive CCW from east)
    if (angle_deg < 0.0f) {
        angle_deg += 360.0f;
    }

	// No shift needed; already 0° east, counterclockwise
    return angle_deg;
}
