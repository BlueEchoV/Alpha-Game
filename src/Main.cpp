	...
	while (Globals::running) {
		...
		if (!player->dead) {
			...
			if (player_moving) {
				...
				// NOTE: Check if the player movement is mostly 'forward' (same as aim) or 'backward' (opposite to aim)
				//		 using the dot product to see how aligned the two directions are. If it's less than 0, it's backwards.
			...
			player->passive->update_ability(delta_time);
