...
void render(Game_Data& game_data, float delta_time) {
	...
	MP_Rect player_draw_rect = {
		(int)game_data.player.rb.pos_ws.x - game_data.player.w / 2,
		(int)game_data.player.rb.pos_ws.y - game_data.player.h / 2,
		game_data.player.w, game_data.player.h};
	// This is the camera space position of the player. Not the world space position.
	V2 player_pos_cs = convert_ws_to_cs({ (float)player_draw_rect.x, (float)player_draw_rect.y }, game_data.camera.pos_ws);
	player_draw_rect.x = (int)player_pos_cs.x; 
